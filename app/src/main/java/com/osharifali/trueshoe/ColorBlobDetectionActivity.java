...
public class ColorBlobDetectionActivity extends Activity implements OnTouchListener, CvCameraViewListener2 {
    ...
    @BindView(R.id.capture) Button capture;
...
//        mOpenCvCameraView.addTouchables(views);
    ...
    protected void saveImage(View view) {
        ...
        saveImage(resultBitmap, "plswork");
        List<MatOfPoint> cons = mDetector.getContours();
        ArrayList<Double> xPoints = new ArrayList<>();
        ArrayList<Double> yPoints = new ArrayList<>();


        for (MatOfPoint i : cons) {
            for (Point p :i.toList()) {
                Log.d("IMG POINTS", p.toString());
                xPoints.add(p.x);
                yPoints.add(p.y);
            }
        }
        double minX = Collections.min(xPoints);
        double maxX = Collections.max(xPoints);
        double xDiff = maxX - minX;

        double minY = Collections.min(yPoints);
        double maxY = Collections.max(yPoints);
        double yDiff = maxY - minY;

        Log.d("IMG POINTS", "XDIFF: " + xDiff + " YDIFF: " + yDiff);
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
        ...
        if (mIsColorSelected) {
            mDetector.setColorRadius(new Scalar(360,2,20));
